...
void insertLastDosen(listDosen &Ld, adrDosen p) {
    if (searchDosenByNIP(Ld, p->info.NIP)!=NULL || searchDosenByKode(Ld, p->info.kodeDosen)!=NULL) {
        // tidak masuk ke list jika sudah digunakan
        cout << "NIP atau Kode Dosen sudah digunakan" << endl;
        delete p;
    } else if (Ld.first==NULL && Ld.last==NULL) {
        Ld.first=p;
        Ld.last=p;
    } else {
        Ld.last->next=p;
